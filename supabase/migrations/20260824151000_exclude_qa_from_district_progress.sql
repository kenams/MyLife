-- Fix (trouvé Phase 7) : district_progress (source du classement des
-- quartiers) était incrémenté pour TOUT claim_mission_reward, y compris les
-- comptes QA — contrairement à leaderboard_players qui les exclut déjà via
-- qa_test_accounts. Un compte QA qui teste une mission plusieurs fois
-- gonflait artificiellement le score public d'un quartier.
-- Fix : ne plus contribuer à district_progress si l'appelant est un compte
-- QA connu (badges/notifications restent inchangés, utiles pour les tests).
create or replace function public.claim_mission_reward(p_mission_id uuid)
returns season_reward_ledger
language plpgsql
security definer
...
begin
  ...
  select exists(select 1 from public.qa_test_accounts qa where qa.user_id = auth.uid()) into v_is_qa;
  ...
  if v_mission.district_id is not null and not v_is_qa then
